...
public class Order
{
    ...
    // Payment info
    [DynamoDBProperty] public required string PaymentIntentId { get; set; } // StripeID can be retrieved via payment intent
    
    
    [DynamoDBProperty] public required Guid CustomerId { get; set; } // 1-m relationship on customer GUID TODO make this a range key

    [DynamoDBProperty] public string OrderStatus { get; set; } = "Pending"; // Order status can be one of the following: "Pending", "Processing", "Shipped", "Delivered", "Cancelled"
